locals {
  superhero_rds_name      = "superhero${local.env_human}"
  superhero_postgres_tags = merge({ "Name" : local.superhero_rds_name }, { "ebs-backup" : "true" }, local.standard_tags)
}
...
module "superhero-backend-postgres-sg" {
  ...
  tags         = local.superhero_postgres_tags
}
...
module "superhero-backend-postgres" {
  ...
  password = local.config.superhero_rds_password
  port     = 5432
  ...
  monitoring_role_name                  = local.superhero_rds_name
